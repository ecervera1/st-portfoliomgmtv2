...
def scrape_stock_data(ticker):
    ...
    data = {
        ...
        "Revenue Growth": f"{info.get('revenueGrowth') * 100:.2f}%" if info.get('revenueGrowth') is not None else None,
        "Earnings Growth": f"{info.get('earningsGrowth') * 100:.2f}%" if info.get('earningsGrowth') is not None else None,
...
if st.sidebar.checkbox('Portflio Optimizer', value=False):
    ...
    end_date = st.text_input("End Date (YYYY-MM-DD)", "2024-03-19")
    #default_end_date = datetime.today().date()
